...
{
	return image->pixels[coords.m_x * image->image_data.width + coords.m_y];
}

void image_set_pixel(s_vector2_t coords, s_color_t pixel, s_image_t *image)
{
	image->pixels[coords.m_x * image->image_data.width + coords.m_y] = pixel;
}
...
{
	if (coords.m_x >= (__s64)image->image_data.height ||
		coords.m_y >= (__s64)image->image_data.width)
		return 0;
